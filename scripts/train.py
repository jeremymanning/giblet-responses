...
def main():
    ...
    parser.add_argument(
        "--data_dir", type=str, default=None, help="Override data directory from config"
    )
    ...
    model = create_autoencoder(
        ...
        ),  # Issue #30: Frame skipping used instead
        frame_skip=data_config.get(
            "frame_skip", 2
        ),  # Issue #30: Memory optimization via frame skipping
    ...
    if is_main_process and model_config.get("gradient_checkpointing", False):
        print("\nGradient checkpointing enabled:")
        print("  Reduces activation memory by ~30-50%")
        print(
            "  Training will be ~30-40% slower (recomputes activations during backward pass)"
        )
        print("  Recommended for very large models on GPUs with limited memory")
