...
def create_network_diagram(
    ...
    if title is None:
        ...
        title = (
            f"Network Architecture ({len(layers)} layers, {total_params:,} parameters)"
        )
    ax.text(
        5,
        len(layers) + 1.5,
    ...
    if legend:
        legend_elements = [
            mpatches.Patch(
                facecolor=(0.2, 0.4, 0.8, 0.7), edgecolor="black", label="Video Encoder"
            ),
            mpatches.Patch(
                facecolor=(0.8, 0.4, 0.2, 0.7), edgecolor="black", label="Audio Encoder"
            ),
            mpatches.Patch(
                facecolor=(0.2, 0.8, 0.4, 0.7), edgecolor="black", label="Text Encoder"
            ),
            mpatches.Patch(
                facecolor=(0.8, 0.2, 0.8, 0.7), edgecolor="black", label="Bottleneck"
            ),
            mpatches.Patch(
                facecolor=(0.9, 0.6, 0.3, 0.7), edgecolor="black", label="Decoder"
            ),
            mpatches.Patch(
                facecolor=(0.6, 0.6, 0.6, 0.7), edgecolor="black", label="Generic Layer"
            ),
        ]
        ax.legend(
...
def create_model_summary(model: nn.Module, output_path: Optional[str] = None) -> str:
    ...
    for layer in layers:
        ...
        if len(name) > 38:
            name = name[:35] + "..."
        summary_lines.append(f"{name:<40} {layer['type']:<20} {layer['params']:>15,}")

    summary_lines.extend(["=" * 80, f"Total parameters: {total_params:,}", "=" * 80])
