...
class MultimodalDataset(Dataset):
    ...
    def _preprocess_data(self):
        ...
        print("  Using dummy text features (sentence-transformers unavailable)")
        ...
        if self.mode == "per_subject":
            ...
            sample = {
                "video": torch.from_numpy(
                    self.video_features[subject_idx, tr_idx]
                ).float(),
                "audio": torch.from_numpy(
                    self.audio_features[subject_idx, tr_idx]
                ).float(),
                "text": torch.from_numpy(
                    self.text_features[subject_idx, tr_idx]
                ).float(),
                "fmri": torch.from_numpy(
                    self.fmri_features[subject_idx, tr_idx]
                ).float(),
