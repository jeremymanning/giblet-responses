...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize MultimodalAutoencoder with torchview"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="papers/figs/source/autoencoder_architecture",
    ...
    print(
        f"  Orientation: {args.orientation} ({'horizontal' if args.orientation == 'LR' else 'vertical'})"
    )
    ...
    print(
        "  python examples/visualize_autoencoder_torchview.py --depth=3 --expand-nested --orientation=LR"
    )
    print("\n  # Publication quality PDF")
    print(
        "  python examples/visualize_autoencoder_torchview.py --format=pdf --orientation=LR"
    )
