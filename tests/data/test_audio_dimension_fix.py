...
class TestEnCodecDimensionFix:
    ...
    def test_consistent_dimensions_small(self, processor, sherlock_video):
        ...
        for i in range(5):
            assert features[i].shape == (896,), f"TR {i} wrong shape: {features[i].shape}"
    ...
    def test_different_bandwidths(self, sherlock_video):
        ...
        for bandwidth, n_codebooks, expected_features in bandwidth_configs:
            ...
            assert features.shape == (5, expected_features), \
                f"Bandwidth {bandwidth}: expected shape (5, {expected_features}), got {features.shape}"

            assert all(metadata['n_codebooks'] == n_codebooks), \
