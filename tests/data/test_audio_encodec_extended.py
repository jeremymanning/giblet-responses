...
from giblet.data.audio import ENCODEC_AVAILABLE
...
class TestEnCodecRealData:
    ...
    def test_encodec_sherlock_5trs(self, audio_processor, sherlock_video_path):
        ...
        features, metadata = audio_processor.audio_to_features(
            sherlock_video_path, max_trs=5, from_video=True
        )
    ...
    def test_encodec_sherlock_100trs(self, audio_processor, sherlock_video_path):
        ...
        features, metadata = audio_processor.audio_to_features(
            sherlock_video_path, max_trs=100, from_video=True
        )
    ...
    def test_encodec_dimension_consistency(self, audio_processor, sherlock_video_path):
        ...
        for tr_idx in range(features.shape[0]):
            ...
            assert (
                tr_codes.min() >= 0
            ), f"TR {tr_idx} has negative codes: {tr_codes.min()}"
            assert (
                tr_codes.max() < 1024
            ), f"TR {tr_idx} has codes >= 1024: {tr_codes.max()}"

        print("\n✓ Dimension consistency verified for 50 TRs")
        print("  All TRs have shape (896,)")
        print("  All codes in range [0, 1023]")

    @pytest.mark.skipif(not STOI_AVAILABLE, reason="pystoi not available")
    def test_encodec_quality_sherlock(
        self, audio_processor, sherlock_video_path, tmp_path
    ):
        ...
        features, _ = audio_processor.audio_to_features(
            sherlock_video_path, max_trs=max_trs, from_video=True
        )
    ...
    def test_encodec_round_trip_sherlock(
        self, audio_processor, sherlock_video_path, tmp_path
    ):
        ...
        assert (
            7.0 <= duration <= 8.0
        ), f"Duration {duration:.2f}s not in expected range [7.0, 8.0]"
