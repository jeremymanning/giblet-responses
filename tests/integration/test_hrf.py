...
class TestCanonicalHRF:
    ...
    def test_hrf_peak_latency(self):
        ...
        assert (
            4.5 <= latency <= 7.0
        ), f"Peak latency {latency} not in expected range [4.5, 7.0]"
...
class TestConvolveWithPadding:
    ...
    def test_convolve_with_padding_reduces_edge_effects(self):
        ...
        convolved_padding = convolve_with_padding(
            features, tr=1.5, padding_duration=15.0
        )
...
class TestVisualization:
    ...
    def test_plot_multifeature_convolution(self, test_data_dir):
        ...
        for i in range(3):
            # Stimulus
            axes[i, 0].bar(time, features[:, i], width=tr, alpha=0.7, color=colors[i])
            ...
            axes[i, 1].fill_between(time, convolved[:, i], alpha=0.3, color=colors[i])
            if i == 0:
        ...
        output_dir.mkdir(exist_ok=True)
        plt.savefig(
            output_dir / "multifeature_convolution.png", dpi=100, bbox_inches="tight"
        )
        plt.close()
