...
# Plot 1: All layers
fig.suptitle('Autoencoder Architecture: Dimension Flow Analysis', fontsize=16, fontweight='bold')
...
bars = ax1.bar(positions, dims, color=colors, alpha=0.7, edgecolor='black', linewidth=1.5)
...
for i, (label, dim, layer_type) in enumerate(layers):
    if 'L6' in label:
        ...
        ax1.text(i, dim, f'{dim:,}\n(L6)', ha='center', va='bottom',
                fontsize=10, fontweight='bold', color='red')
    elif 'L7' in label:
        bars[i].set_edgecolor('darkgreen')
        bars[i].set_linewidth(3)
        ax1.text(i, dim, f'{dim:,}\n(TRUE MIN)', ha='center', va='bottom',
...
# Color main pathway
main_colors = ['#4CAF50' if i == 0 or i == len(main_pathway)-1
               else '#FF5722' if i == 5   # L6
               else '#00C853' if i == 6   # L7 (true min)
               else '#2196F3' if i < 5
               else '#9C27B0'
               for i in range(len(main_pathway))]

bars2 = ax2.bar(main_positions, main_dims, color=main_colors, alpha=0.7,
...
ax2.plot(main_positions, main_dims, 'k--', alpha=0.4, linewidth=2, marker='o', markersize=8)

# Annotate bottleneck issue
l6_idx = 5
l7_idx = 6
ax2.annotate('', xy=(l7_idx, main_dims[l7_idx]), xytext=(l6_idx, main_dims[l6_idx]),
            arrowprops=dict(arrowstyle='<->', color='red', lw=2))
ax2.text((l6_idx + l7_idx) / 2, (main_dims[l6_idx] + main_dims[l7_idx]) / 2,
...
ax2.text(l6_idx, main_dims[l6_idx], f'{main_dims[l6_idx]:,}\n(Spec: Smallest)',
        ha='center', va='bottom', fontsize=10, fontweight='bold', color='red')

# Highlight L7
bars2[l7_idx].set_edgecolor('darkgreen')
bars2[l7_idx].set_linewidth(3)
ax2.text(l7_idx, main_dims[l7_idx], f'{main_dims[l7_idx]:,}\n(Actually Smallest!)',
...
plt.tight_layout()
plt.savefig('/Users/jmanning/giblet-responses/notes/dimension_flow.png', dpi=300, bbox_inches='tight')
...
for label, dim in main_pathway:
    marker = " ★ SPEC BOTTLENECK" if label == 'L6' else " ★ TRUE BOTTLENECK" if label == 'L7' else ""
