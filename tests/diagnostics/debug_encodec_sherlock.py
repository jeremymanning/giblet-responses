...
def debug_encodec_extraction():
    ...
    for max_trs in test_trs:
        ...
        try:
            ...
            print(f"   ✓ All TRs consistent: {all(features[i].shape == features[0].shape for i in range(len(features)))}")
            ...
            try:
                ...
                # Load audio
                y, sr = librosa.load(video_path, sr=processor.encodec_sample_rate, mono=False)
                if y.ndim > 1:
                    y = np.mean(y, axis=0)
                ...
                print(f"      Codebooks (bandwidth {processor.encodec_bandwidth} kbps): 8")
                ...
                for tr_idx in range(min(3, max_trs)):
                    ...
                    print(f"      TR {tr_idx}: start_frame={start_frame}, end_frame={end_frame}, shape={tr_codes.shape}")

                    # Check if temporal dimension matches
                    if tr_codes.shape[1] != frames_per_tr:
                        print(f"         WARNING: Expected {frames_per_tr} frames, got {tr_codes.shape[1]}")

                    # Check codebook dimension
                    if tr_codes.shape[0] != 8:
                        print(f"         WARNING: Expected 8 codebooks, got {tr_codes.shape[0]}")
