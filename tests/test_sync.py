...
class TestResampleFeatures:
    ...
    def test_resample_downsample(self):
        ...
        assert result.shape == (0, 1) or result.shape == (50, 1), f"Unexpected shape: {result.shape}"
...
class TestAlignAllModalities:
    ...
    def test_output_shapes_match_n_trs(self, sample_data):
        ...
        for modality in ['video', 'audio', 'text', 'fmri']:
            assert result[modality].shape[0] == n_trs, \
                f"{modality} first dimension doesn't match n_trs"
            assert len(result[modality].shape) == 2, \
                f"{modality} should be 2D array"
    ...
    def test_finite_outputs(self, sample_data):
        ...
        for modality in ['video', 'audio', 'text', 'fmri']:
            assert np.all(np.isfinite(result[modality])), \
...
class TestGetAlignmentInfo:
    ...
    def test_get_info(self):
        ...
        expected_keys = [
            'n_trs', 'video_trs_aligned', 'audio_trs_aligned', 'text_trs_aligned',
            'fmri_trs_aligned', 'video_features', 'audio_features', 'text_features',
            'fmri_features', 'video_orig_trs', 'audio_orig_trs', 'text_orig_trs',
            'fmri_orig_trs'
        ]
